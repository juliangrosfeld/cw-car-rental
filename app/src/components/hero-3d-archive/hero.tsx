...
function ScrollHero({ showCanvas, staticCamera }: { showCanvas: boolean; staticCamera: boolean }) {
  ...
  // A stage only takes clicks while it is actually on screen, and is pulled out
  // of the paint entirely (`visibility: hidden`) the moment it fades below a
  // whisker of opacity — the hard guarantee that only one caption is ever
  // visible, even if a scroll value hasn't settled yet.
  const s1Pointer = useTransform(s1Opacity, (v) => (v > 0.35 ? 'auto' : 'none'))
  const s3Pointer = useTransform(s3Opacity, (v) => (v > 0.35 ? 'auto' : 'none'))
  const s1Vis = useTransform(s1Opacity, (v) => (v > 0.02 ? 'visible' : 'hidden'))
  const s2Vis = useTransform(s2Opacity, (v) => (v > 0.02 ? 'visible' : 'hidden'))
  const s3Vis = useTransform(s3Opacity, (v) => (v > 0.02 ? 'visible' : 'hidden'))
      ...
      <div className="sticky top-0 h-dvh overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 z-10">
          <div className="mx-auto h-full max-w-[1160px] px-5 md:px-8">
            {/* Each caption is a plain motion.div driven directly by a scroll
                MotionValue — the pattern that actually commits opacity to the
                DOM every frame. `visibility` is hard-gated to `hidden` the
                instant a stage's opacity drops near zero, so a not-yet-settled
                opacity value can never leave two captions painted at once. The
                fade windows are disjoint (s1 ends at 0.28, s2 lives 0.34–0.66,
                s3 starts at 0.74), giving a clean crossfade with no overlap. */}

            {/* Stage 1: the welcome, on screen at scroll zero. */}
            <motion.div
              style={{ opacity: s1Opacity, y: s1Y, pointerEvents: s1Pointer, visibility: s1Vis }}
              data-stage="1"
            ...
            <motion.div
              style={{ opacity: s2Opacity, visibility: s2Vis }}
              data-stage="2"
            ...
            <motion.div
              style={{ opacity: s3Opacity, y: s3Y, pointerEvents: s3Pointer, visibility: s3Vis }}
              data-stage="3"
